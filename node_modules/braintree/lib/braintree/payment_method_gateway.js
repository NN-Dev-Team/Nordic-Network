...
var AndroidPayCard, ApplePayCard, CoinbaseAccount, CreditCard, Gateway, PayPalAccount, PaymentMethodGateway, PaymentMethodNonce, UnknownPaymentMethod, UsBankAccount, Util, VenmoAccount, deprecate, exceptions, querystring,
...
PaymentMethodGateway = (function(_super) {
  ...
  PaymentMethodGateway.prototype.grant = function(token, attributes, callback) {
    var grantOptions;
    if (token.trim() === '') {
      return callback(exceptions.NotFoundError("Not Found"), null);
    } else {
  ...
  PaymentMethodGateway.prototype["delete"] = function(token, options, callback) {
    var queryParam;
    if (!callback) {
      callback = options;
      options = null;
    }
    Util.verifyKeys(this._deleteSignature(), options, deprecate);
    queryParam = options != null ? "?" + querystring.stringify(Util.convertObjectKeysToUnderscores(options)) : "";
    return this.gateway.http["delete"]("" + (this.config.baseMerchantPath()) + "/payment_methods/any/" + token + queryParam, callback);
  };

  PaymentMethodGateway.prototype._deleteSignature = function() {
    return {
